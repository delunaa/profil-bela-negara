          ...
          <div className="hidden md:flex items-center space-x-8">
            {navigation.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="text-sm font-medium text-slate hover:text-navy transition-all duration-300 hover:-translate-y-0.5 focus:outline-none focus:ring-2 focus:ring-red rounded px-2 py-1"
              >
                {item.name}
              </Link>
            ))}
            <Link
              href="/kontak"
              className="bg-navy text-white px-5 py-2 rounded-full text-sm font-medium hover:bg-navy/90 transition-all duration-300 shadow-md hover:shadow-lg hover:-translate-y-0.5 focus:outline-none focus:ring-2 focus:ring-red min-h-[44px] flex items-center"
            >
              Hubungi Kami
            </Link>
          </div>
